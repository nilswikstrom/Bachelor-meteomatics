## Script to fetch METGC files from MET (Meteorologisk institutt)
...
OUTPUT  = "MetInst/API/metgc_filer"   # Mappe filene lagres i (opprettes automatisk)
HEADERS = {"User-Agent": "Session-name mail@example.com"}
KJØRETIDER = [3, 9, 15, 21]  # UTC-timer METGC produseres
...
for i, ts in enumerate(TIDSPUNKT, 1):
    ...
    filnavn  = f"metgc_{AREA}_{api_tid.replace(':', '').replace('Z', 'UTC')}.gmc"
    ...
    resp = requests.get("https://api.met.no/weatherapi/metgc/1.0/",
