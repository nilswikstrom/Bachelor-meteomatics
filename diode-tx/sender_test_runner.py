"""
sender_test_runner.py — Automated test runner for UDP file transfers.

Runs (N files) x 3 redundancy levels x 25 runs per configuration.
Reads all files from the METCM/ subfolder — no hardcoded filenames.
...
File layout expected:
    script/
    ├── sender_test_runner.py
    ├── sender_udp.py
    ├── METCM/
    │   ├── 2026-02-23T12-01-10Z_MM-670M-39_Standard
    │   ├── 2026-02-23T12-01-10Z_MM-670M-39_Dropzone
    │   ├── 2026-02-23T12-01-10Z_MM-670M-39_Ballistic
    │   └── 2026-02-23T12-01-10Z_MM-670M-39_Base
    └── logs/
        └── sender_baseline.csv
"""
...
def load_test_files() -> list:
    ...
    for subfolder in subfolders:
        folder_path = os.path.join(metcm_dir, subfolder)
    ...
    if not files:
        print(f"[ERROR] No files found in {metcm_dir}")
        sys.exit(1)

    print(f"[SETUP] Found {len(files)} file(s) in METCM/:")
